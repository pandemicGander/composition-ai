...
# Optional: Set memory growth for GPU so TF doesn't grab all GPU memory up front.
...
def load_data_from_file(file_path, seq_length=100):
    ...
    if len(data) <= seq_length:
        return [], []  # Not enough data to form a single sequence
...
def count_total_sequences(file_paths, seq_length=100):
    ...
    for fp in file_paths:
        ...
        # Each valid sequence has length seq_length, so we form (len(data) - seq_length) sequences
...
def build_lstm_model(input_shape):
    ...
    model = Sequential([
        ...
        Dropout(0.2),
        LSTM(32),
        Dense(108, activation='sigmoid')  # 108 outputs, as in your original code
    ])
...
def predict_next_sequence(model, input_sequence):
    ...
    """
    input_sequence = np.expand_dims(input_sequence, axis=0)  # shape (1, seq_length, features)
    prediction = model.predict(input_sequence)[0]           # shape (features,)
...
if __name__ == '__main__':
    ...
    model_save_path = 'models/lstm_exact_harmony_model.keras'  # Adjust path if desired
    ...
    # 2. Split file paths into train/val at the file level
    ...
    # 5. Create data generators for train and validation
    ...
    # 6. Build the model
    #    We assume num_features = 108 for the output, but your input dimension
    #    might still be something else (e.g., 20). For a real scenario, ensure
    #    the first layer's input_shape matches your actual input dimension.
    #    If each step has 108 features, set input_shape=(seq_length, 108).
    #
    #    If your data actually has fewer input features, adjust input_shape accordingly.
    #    For this example, we'll assume input_shape=(seq_length, 108).
    model = build_lstm_model((seq_length, 108))
    ...
    if val_files:
        ...
        if sequences:
            example_sequence = sequences[-1]  # Last sequence from this file
